...
本指南帮助你从原有的单体架构（历史的 `get_up.py`）迁移到新的模块化架构。新架构解决了 POSTMORTEM.md 中识别的关键问题，提供了更好的可维护性、扩展性和可观测性。注意：当前仓库默认不再包含旧版脚本，仅在你本地仍保留旧脚本时需要参考迁移步骤。
...
1. **保留原系统作为备选**：在确认新系统稳定之前，如仍保留旧脚本，则保留 `get_up.py`
...
```bash
# 主要变量
export GEMINI_IMAGEN_API_KEY="your-key"
