...
### 项目愿景
构建一个智能化的个人习惯养成系统，通过多模态内容生成实现早起打卡的数字化仪式感。（历史文档中包含单词学习设想，当前代码库已聚焦早起打卡链路）

### 核心功能实现
1. **早起打卡系统** - 诗词获取 + AI图片生成 + 多平台发布
2. **单词学习系统（历史设想/旧脚本）** - 词汇获取 + 故事生成 + 语音合成
...
# 支持多种部署环境
GEMINI_IMAGEN_API_BASE = get_env_or_default("GEMINI_IMAGEN_API_BASE", "https://ai.huan666.de/v1")
```
...
- `get_up.py` (809行) - 早起打卡功能（历史脚本）
- `cichang.py` (199行) - 单词学习功能（历史脚本）
...
GEMINI_IMAGEN_API_KEY = os.environ.get("GEMINI_IMAGEN_API_KEY")  # 在另一个位置（当前实现）
```
...
class ContentGenerationService:
    def __init__(self, config: ContentConfig):
        self.generators = [
            GeminiImagenGenerator(config.gemini_imagen),
...
logger.warning(f"Gemini Imagen generation returned no result (attempt {attempt + 1})")
logger.error("All Gemini Imagen generation attempts failed")
...
**决策**: 同时集成 Gemini Imagen（OpenAI 兼容）、OpenAI、Unsplash 等多个API
**结果**: 成功，提高了系统可靠性
...
├── 2025-01-20/
│   └── gemini_imagen_1768877843_0.jpg
```
...
**性能瓶颈**:
1. **外部API调用**: Gemini Imagen 图片生成是主要瓶颈
...
async def generate_content_parallel(sentence: str):
    tasks = [
        asyncio.create_task(generate_gemini_imagen(sentence)),
...
```python
# 通过环境变量管理敏感信息
GEMINI_IMAGEN_API_KEY = os.environ.get("GEMINI_IMAGEN_API_KEY")
